...
public class TrainRenderer {
    ...
    // Layout constants (shared with TrainScene via package visibility)
    static final float DOOR_X     = 320f;
    static final float DOOR_Y     = 105f; // height of the train band
    static final float DOOR_WIDTH = 50f;  // half-width of the door gap
    ...
    public void render(float W, float H, float doorOpenAmount, boolean lost) {
        ...
        if (lost) renderer.drawRect(0, 0, W, H, COL_ENDGAME_OVERLAY);
    }
    ...
    private void drawPlatform(float W, float H) {
        ...
        for (float x = 80f; x < W; x += 80f)
            renderer.drawLine(x, 0, x, H - DOOR_Y, COL_GRID);
    }

    private void drawTrain(float W, float H, float doorOpenAmount) {
        float trainY = H - DOOR_Y;
        renderer.drawRect(0, trainY, W, DOOR_Y, COL_TRAIN_BODY);
    ...
    private void drawWindows(float H) {
        ...
        for (float x : windowX) {
            renderer.drawRect(x, H - DOOR_Y + 40f, 80f, 48f, COL_WIN_BORDER);
            renderer.drawRect(x + 3f, H - DOOR_Y + 43f, 74f, 42f, COL_WIN_GLASS);
            renderer.drawRect(x + 3f, H - DOOR_Y + 75f, 22f, 8f, COL_WIN_SHINE);
        }
    }

    private void drawDoor(float trainY, float doorOpenAmount) {
        // Background recess
        renderer.drawRect(DOOR_X - DOOR_WIDTH - 4f, trainY, DOOR_WIDTH * 2f + 8f, DOOR_Y, COL_DOOR_BG);

        // Animated sliding panels
        float panelWidth = DOOR_WIDTH * (1f - doorOpenAmount);
        renderer.drawRect(DOOR_X - DOOR_WIDTH, trainY, panelWidth, DOOR_Y - 8f, COL_DOOR_PANEL);
        renderer.drawRect(DOOR_X - DOOR_WIDTH + panelWidth * 0.6f, trainY, 2f, DOOR_Y - 8f, COL_DOOR_LINE);
        renderer.drawRect(DOOR_X + DOOR_WIDTH - panelWidth, trainY, panelWidth, DOOR_Y - 8f, COL_DOOR_PANEL);
        renderer.drawRect(DOOR_X + DOOR_WIDTH - panelWidth * 0.4f, trainY, 2f, DOOR_Y - 8f, COL_DOOR_LINE);

        // Door frame and sill
        renderer.drawRect(DOOR_X - DOOR_WIDTH - 6f, trainY, 6f, DOOR_Y, COL_DOOR_FRAME);
        renderer.drawRect(DOOR_X + DOOR_WIDTH, trainY, 6f, DOOR_Y, COL_DOOR_FRAME);
        renderer.drawRect(DOOR_X - DOOR_WIDTH - 6f, trainY, DOOR_WIDTH * 2f + 12f, 9f, COL_DOOR_SILL);
    }

    private void drawTactileStrip(float W, float H) {
        renderer.drawRect(0, H - DOOR_Y - 10f, W, 10f, COL_TACTILE);
        for (float x = 20f; x < W; x += 25f)
            renderer.drawCircle(x, H - DOOR_Y - 5f, 3.5f, COL_BUMP);
    }
    ...
    private void drawWaitingMarkers(float H) {
        ...
        renderer.drawRect(DOOR_X + DOOR_WIDTH + 1f, my, 5f, 22f, COL_MARKER);
    }
}
